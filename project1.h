...
// Helper functions and data structures created by us
// Map to hold label names and their corresponding instruction line numbers
static std::unordered_map<std::string, int> label_map;

// Map to hold static variable names and their corresponding memory addresses and sizes
static std::unordered_map<std::string, int> staticMemoryMap;
...
std::vector<std::string> split(const std::string &s, const std::string &split_on) {
    ...
    while(cur_pos != std::string::npos) {
        ...
        //std::cout << s.substr(new_pos,cur_pos-new_pos) << std::endl;
    }
    //std::cout << " " << std::endl;
    return split_terms;
}
...
int encode_Itype(int opcode, int rs, int rt, int constant) {
    if (constant < 0) {
        //std::cout << (opcode << 26) + (rs << 21) + (rt << 16) + constant << std::endl;
        return (opcode << 26) + (rs << 21) + (rt << 16) + (65535 + constant + 1); // 65535 + constant + 1 is the two's complement conversion
    }
